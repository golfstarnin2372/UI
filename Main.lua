...
local Instance_new = (gethui and function(...)
									...
									if multiselect then
										...
										if selectedOption ~= v then
											...
											if type(selectedOption) == "table" then
												local selectionText = ""
												for i,v in pairs(selectedOption) do
													if i == 1 then
														selectionText = selectionText..tostring(v)
													else
				...
				function update()
					...
					if dropdownSelection.Text ~= sstr then
					    if type(selectedOption) == "table" then
					       ...
					       if #Nsstr > 22 then
					           Nsstr = Nsstr:sub(1,22)
					           if Nsstr:sub(#Nsstr,#Nsstr) == " " then
					               Nsstr = Nsstr:sub(1,#Nsstr-2)
					           elseif Nsstr:sub(#Nsstr,#Nsstr) == "," then
					               Nsstr = Nsstr:sub(1,#Nsstr-1)
					           end
					           Nsstr = Nsstr:sub(1,22).."..."
					       end
					       if string.gsub(Nsstr," ","") == "" then
					           Nsstr = blankstring or "Select Item(s)"
					       end
					       sstr = Nsstr
					    end
						dropdownSelection.Text = sstr
